...
contract UStaking is Ownable{
    ...
    uint256 public cashBackReward = 2;
    uint256 public refWalletReward = 25;
    ...
    constructor(address _token,address _refWallet) {
        token = IBEP20(_token);
    ...
    function refWalletUpdate(address newRefWallet) public onlyOwner {
    ...
    function stake(uint256 stakeType,uint256 amount) external {
        ...
        require(idStore[msg.sender].insert(internalStakeTicket),"not added to the set");
        stakeStore[internalStakeTicket] = user({
            ...
            unStakeTime: block.timestamp.add(stakeDuration[stakeType]),
            stakeType: stakeType,
            withdrawStatus: false,
            cashBackStatus: false
        });
        
        token.mint(refWallet,amount.mul(refWalletReward).div(100));
        uint256 mintAmount = amount.mul(stakeReward[stakeType]).div(100).add(amount.mul(cashBackReward).div(100));
    ...
    function claim(uint256 stakeId) external {
        ...
        uint256 reward = pendingRewards(stakeId).sub(store.rewardDebt);
        
        if(store.rewardDebt.add(reward) > store.maxRewards) {
            reward = store.maxRewards.sub(store.rewardDebt);
        }
         
        store.rewardDebt = store.rewardDebt.add(reward);
        token.safeTransfer(store.user,reward);
        totalRewardEmission = totalRewardEmission.add(reward);
    ...
    function pendingRewards(uint256 stakeId) public view returns(uint256) {
        if(stakeStore[stakeId].rewardDebt < stakeStore[stakeId].maxRewards){
            uint256 stakeTime = (block.timestamp - stakeStore[stakeId].stakeTime).mul(1e12).div(stakeDuration[stakeStore[stakeId].stakeType]);
            return stakeStore[stakeId].stakeAmount.mul(stakeTime).mul(stakeReward[stakeStore[stakeId].stakeType]).div(100e12);
        }else{
            return 0;
        }
    }
    ...
    /// @param stakeId id stake which was assigned in the function stake
    function cashBack(uint256 stakeId) public {
        user storage store = stakeStore[stakeId];
        require(isContains(msg.sender,stakeId), "invalid stake id");
        require(!store.cashBackStatus, "cashback already claimed");
        
        store.cashBackStatus = true;
        token.safeTransfer(store.user,store.stakeAmount.mul(cashBackReward).div(100));
    }
    
    /// @notice The function of checking the presence of stake by address and id
    /// @dev The function of checking the presence of stake by address and id
    /// @param account the address of the user who has already staked
    /// @param stakeId id stake which was assigned in the function stake
    /// @return return true if  there is user with address and id, else return false
    function isContains(address account,uint256 stakeId) public view returns (bool) {
        return idStore[account].contains(stakeId);
    }
    ...
    function stakeTotalIds(address account) public view returns (uint256[] memory) {
    ...
    function getUserData(address userAddress) public view returns (
